 ...
 * ever receives.
 *
 * It is load-bearing for the download proxy: some URL shapes (an unencoded `/` inside the
 * `:bookId` segment, a segment past `maxParamLength`) are refused by find-my-way BEFORE any route
 * hook runs, so they never reach the handler's guard or its id grammar. What they must still be is
 * CONTENT-FREE — our envelope, with a message interpolating only the caller's own URL, and no
 * upstream call. Sharing one registrar is what keeps the harness and production from drifting on
 * that.
 *
