...
describe('the happy path, end to end over real sockets', () => {
  it('saves an address, sends it, and records ONE redacted audit row', async () => {
    ...
    // AC11 — the self-scoped carrier. This endpoint is the ONE documented exemption from the body
    // sweep: it legitimately returns the caller's OWN Kindle address. Its headers and the captured
    // logs are still swept (an empty body haystack keeps the other two surfaces asserted).
    ...
    s.sweep('PATCH /api/me (headers + logs only — the body is the documented carrier)', saved, '');
    ...
    s.sweep('GET /api/me (headers + logs only — the body is the documented carrier)', me, '');
